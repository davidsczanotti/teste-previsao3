...
def _sliding_window_features(
    ...
    ma_short = _moving_average(close_series, ma_short_window, ma_type)
    ma_mid = _moving_average(close_series, ma_mid_window, ma_type)
    ma_long = _moving_average(close_series, ma_long_window, ma_type)
    ...
    for idx in range(start, len(df)):
        ...
        ma_m = float(ma_mid.iloc[idx]) if not np.isnan(ma_mid.iloc[idx]) else last_close
        ...
        ma_m_prev = float(ma_mid_prev.iloc[idx]) if not np.isnan(ma_mid_prev.iloc[idx]) else ma_m
        ...
        ma_mid_rel_close = (ma_m - last_close) / last_close if last_close != 0 else 0.0
        ma_long_rel_close = (ma_l - last_close) / last_close if last_close != 0 else 0.0
        ma_diff_rel_sm = (ma_s - ma_m) / (abs(ma_m) + 1e-6)
        ma_diff_rel_ml = (ma_m - ma_l) / (abs(ma_l) + 1e-6)
        ma_diff_rel_sl = (ma_s - ma_l) / (abs(ma_l) + 1e-6)
        ma_short_slope = ma_s - ma_s_prev
        ma_mid_slope = ma_m - ma_m_prev
    ...
    extras = {
        ...
        "ma_mid": ma_mid.iloc[start:].ffill().bfill().astype(float).to_numpy(dtype=np.float32),
        ...
        "ma_mid_prev": ma_mid_prev.iloc[start:].ffill().bfill().astype(float).to_numpy(dtype=np.float32),
...
class MaCrossoverEnv:
    """RL environment for MA(7)/MA(40) crossover trading.

    Observation (float32 vector):
      - Per-candle normalised features for the last 7 closed candles (4 each)
      - Aggregates: price momentum + moving-average context (10 dims)
      - Position flags (2)
      Total: 40 dims
    ...
    exec_at_next_open: bool = True,
        ...
        self.ma_short_window = int(ma_short_window)
        self.ma_mid_window = int(ma_mid_window)
        self.ma_long_window = int(ma_long_window)
    ...
    def observation_size(self) -> int:
        return (4 * 7 + 14) + 2  # base features + aggregates + position flags
    ...
    def step(self, action: int) -> StepResult:
        ...
        try:
            if self._ma_short is not None and self._ma_mid is not None and self._ma_long is not None:
                ma_s = float(self._ma_short[self._i])
                ma_m = float(self._ma_mid[self._i])
                ma_l = float(self._ma_long[self._i])
                ma_s_prev = float(self._ma_short_prev[self._i]) if self._ma_short_prev is not None else ma_s
                ma_m_prev = float(self._ma_mid_prev[self._i]) if self._ma_mid_prev is not None else ma_m
        ...
        # Optional auto-entry when exit_only is enabled
        if self.exit_only and self._pos == 0 and heur_action is not None:
            if heur_action == 1:
                if self._bars_since_exit >= self.reopen_cooldown_bars:
                    exec_price = slip_buy(next_open) if self.exec_at_next_open else slip_buy(price)
                    fee = exec_price * self.lot_size * self.fee_rate
                    recent_exit = self._bars_since_exit
                    self._pos = 1
                    self._entry = exec_price
                    self._bars_since_entry = 0
                    reward -= fee
                    reward -= self.action_cost_open
                    if recent_exit <= self.switch_window_bars:
                        reward -= self.switch_penalty
                    self._bars_since_exit = 0
                    info["trade"] = "BUY"
                    trade_executed = True
                else:
                    info["entry_blocked"] = "reopen_cooldown"
            elif heur_action == 3 and not self.long_only:
                if self._bars_since_exit >= self.reopen_cooldown_bars:
        ...
        # When exit_only is enabled and we are still flat, ignore agent actions
        if self.exit_only and self._pos == 0:
            action = 0
