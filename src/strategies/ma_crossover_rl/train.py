...
def train(config: Optional[MaCrossoverRlConfig] = None, model_path: Optional[str] = None) -> Dict[str, Any]:
    cfg = config or MaCrossoverRlConfig()
    env = MaCrossoverEnv(
        symbol=cfg.ticker,
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train RL agent on MA crossover env")
    ...
    parser.add_argument("--gate_on_heuristic", action="store_true", help="Abre posição apenas quando o sinal do crossover MA concordar")
    parser.add_argument("--exit_only", action="store_true", help="Modo apenas saída: entradas seguem o sinal heurístico")
    # resume/warm-start
