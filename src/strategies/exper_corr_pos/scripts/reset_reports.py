...
"""
Reset completo dos artefatos de exper_corr_pos.

Uso:
    poetry run python -m src.strategies.exper_corr_pos.scripts.reset_reports
    poetry run python -m src.strategies.exper_corr_pos.scripts.reset_reports --force

O script remove:
  - src/strategies/exper_corr_pos/reports/train/
  - src/strategies/exper_corr_pos/reports/train/pop/
  - src/strategies/exper_corr_pos/reports/walk_forward/
...
ROOT = Path("src/strategies/exper_corr_pos/reports")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Reseta os artefatos da estratégia exper_corr_pos")
    ...
    subprocess.run(
        [
            ...
            "python",
            "-m",
            "src.strategies.exper_corr_pos.scripts.clean_train_reports",
            "--dir",
            str(TRAIN_DIR),
            "--keep-ep",
            "0",
